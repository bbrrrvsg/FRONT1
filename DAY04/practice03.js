...
let birth = Number(prompt("태어난 년도 입력 : "));

let age = 2025 - birth;

console.log(` result3 나이 : ${age}`);
...
let height = Number(prompt("키 입력 : "))/100;
let weight = Number(prompt("몸무게 입력 : "));

let bmi = weight/((height*height));

console.log(`result4 bmi 지수 : ${bmi}`);

// 문제 5: 관리자 확인
// 아이디와 이메일을 prompt 함수로 입력받습니다.
//  아이디가 'admin'이거나 또는 이메일이 'admin@test.com'이면
//   '관리자'를, 그렇지 않으면 '일반 사용자'를 콘솔에 출력하는 
//   프로그램을 작성하시오.

let id = prompt("아이디");
let em = prompt("이메일");

let login = (id == 'admin' || em == 'admin@test.com') ? '관리자' : '일반 사용자';

console.log(`result5${login}`);
...
// 점수에 따라 아래와 같이 등급을 부여하고 HTML 
// 문서에 <h2> 태그로 출력하는 프로그램을 작성하시오.
// 90점 이상: 'A'
// 80점 이상 90점 미만: 'B'
// 70점 미만: 'C'

let score = Number(prompt("점수를 입력 : "));
let grade = score >=90 ? 'A' : score >=80 ? 'B' :  'C'; 
document.querySelector("h2").innerHTML = grade;


// 문제 7: 청년 할인 이벤트 대상 확인
// 나이를 prompt로 입력받아, 20대 (20세 이상 그리고 29세 이하)에
//  해당하는 경우 "이벤트 대상입니다."를, 그렇지 않으면 
// "이벤트 대상이 아닙니다."를 콘솔에 출력하는 프로그램을 작성하시오.

let age1 = prompt("나이 : ");
let eventage = age1 >= 20 && age1 <= 29 ? '이벤트 대상 입니다' : '이벤트대상이 아닙니다';
console.log('result7'+eventage);



// 문제 8: '좋아요' 카운터 만들기
// '좋아요' 수를 저장하는 likeCount 변수를 3으로 선언하고,
//  증감 연산자(++)를 사용하여 변수의 값을 1 증가시킨
//  뒤, "좋아요: [숫자]" 형식으로 새로운 '좋아요' 수를 콘솔에
//  출력하는 코드를 작성하시오.

let likeCount = 3;
console.log(` result8 좋아요 : ${++likeCount}`) // ++ 뒤에 넣으면 출력후 증가
...
let newtodo = prompt("할일 추가");
todoList.push(newtodo);
console.log(`result9 오늘 할일 : ${todoList}`);
...
console.log(`마지막 대기자 명단은 : ${waitingList[3]}입니다.`)